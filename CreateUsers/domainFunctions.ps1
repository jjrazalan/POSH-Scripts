<# 
.SYNOPSIS 
Function to create user in domain

.DESCRIPTION
Callable function with required parameters to create user in the domain

.AUTHOR
    Josh Razalan
#>
#Function to set various office information in domain
function setOffice {
    param($Office)
...
function createUserEngsys([String]$Firstname, 
    [String]$MI,
    [String]$Lastname, 
    [String]$Username, 
    [String]$Password, 
    ...
    if (Get-ADUser -Filter 'Description -like $Desc' -Server "Domain") {
    ...
    Write-Output "$Username creation successful in ENGSYS.NET."
    Start-Sleep 1
    Return
}	